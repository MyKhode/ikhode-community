<div class="relative">
    <div>
        <div x-data="markdown()" x-ref="markdownX" data-key="{{ $key }}" id="markdown-{{ $key }}" class="relative w-full" x-init="init()">
            ...
            <div class="relative z-40" x-show="section == 'write'">
                ...
                <div class="relative z-30 overflow-hidden @if(isset($style['height'])){{ $style['height'] }}@endif">
                    ...
                    <div wire:ignore x-ref="placeholder" @click="$refs.editor.focus()" id="placeholder-{{ $key }}" x-show="placeholder" class="absolute z-20 h-12 px-3 font-mono leading-loose tracking-tighter border-0 outline-none focus:outline-none sm:x-0 text-lg text-gray-400 transition-opacity duration-200 ease-out" x-cloak>Type '/' for commands </div>
                    <textarea x-ref="editor" id="editor-{{ $key }}" data-key="{{ $key }}" style="height: 150px;" class="editors min-h-[400px] max-h-[2500px] overflow-y-auto w-full mx-auto resize-none overflow-hidden px-5 pt-5 font-mono text-lg leading-loose tracking-tighter text-gray-600 border-0 outline-none focus:outline-none" class="editors @if(isset($style['textarea'])){{ $style['textarea'] }}@else{{ 'w-full h-full mx-auto min-h-screen px-5 md:px-3 pt-5 font-mono leading-loose tracking-tighter border-0 outline-none focus:outline-none sm:x-0 text-lg text-gray-600' }}@endif" placeholder="" data-loaded="false" name="{{ $name }}" x-init="$nextTick(() => $refs.editor.style.height = $refs.editor.scrollHeight + 'px')" x-on:dragenter="$event.preventDefault(); dropFiles=true" x-on:input="$refs.editor.style.height = 'auto'; $refs.editor.style.height = $refs.editor.scrollHeight + 'px'" wire:model.live.blur="content" x-on:blur="@this.call('update', { content: $event.target.value });" x-on:focus="editorEvent($event)" x-on:keypress="editorEvent($event)" x-on:keydown="getCursorXY(); editorEvent($event)" x-on:keyup="getCursorXY(); editorEvent($event)" x-on:click="editorEvent($event)" x-cloak>
                        </textarea>
            ...
            <div x-show="section == 'preview'" wire:target="updateContentPreview" class="@if(isset($style['preview'])){{ $style['preview'] }}@else{{ 'h-full bg-white min-h-screen relative z-30 px-5 pt-5 prose prose-sm md:prose-md lg:prose-sm max-w-none' }}@endif" x-cloak>
                {!! $contentPreview !!}
            </div>
            ...
            window.markdown = function() {
                return {
                    ...
                    upload(evt, key) {
                        ...
                        if (file.size > (parseInt('{{ config("markdownx.image.max_file_size") }}') * 1000) ) {
                            ...
                            if (evt.target.value.length) {
                                ...
                                reader.onloadend = () => {
                                    window.Livewire.dispatch('markdown-x-image-upload', { 'payload': { image: reader.result, name: file.name, key: key, text: "![" + file.name + "](Uploading...)" }});
                                }
                    ...
                    searchGIFModal(event){
                        if (event.keyCode === 13) {
                            ...
                            if(event.target.value.trim() != ""){
                                window.Livewire.dispatch('markdown-x-giphy-search', { 'payload': { search: event.target.value, key: this.$refs.markdownX.dataset.key }});
                            }
                        }
                    },
                    ...
                    selectItem (selected) {
                        ...
                        switch (selected.dataset.suggestion) {
                            ...
                            case 'giphy':
                                    ...
                                    this.$refs.editor.setCaretPosition(this.currentCaretPos.start, this.currentCaretPos.end);
                                    this.replaceSuggestionText(newLine + '');
                                    window.Livewire.dispatch('markdown-x-giphy-load', { 'payload': { key: this.$refs.markdownX.dataset.key }});
                                    this.showModalPop(function(){
                    ...
                    filterList () {
                        ...
                        suggestedList.className += ' mdx-dropdown-scrollbar-hidden';
                    ...
                    editorEvent (e) {
                        ...
                        if ((key === "/" && previousChar == "" && type == "keydown")) {
                            this.toggleSuggestionDropdown();
                            ...
                            switch (key) {
                                ...
                                case 'ArrowDown':
                                    if (type === "keydown") {
                                        e.preventDefault();
                                        // up is 38
                                        this.toggleItem(key === 'ArrowUp' ? "previous" : "next");
                                        // down is 40
                                    }
                                    break;
                                case 'ArrowLeft':
        ...
        <style>
            .mdx-dropdown-scrollbar-hidden::-webkit-scrollbar {
                display: none;
            }

            /* For IE, Edge and Firefox */
            .mdx-dropdown-scrollbar-hidden {
